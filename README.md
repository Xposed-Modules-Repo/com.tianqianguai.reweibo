...
- 可在 ReWeibo 设置中控制“新微博优先排序”；默认开启，关闭后保留微博轻享版原生时间线顺序，且不影响广告过滤、缓存、跳转、删除、阅读位置、断层回补或预加载。
- 过滤广告样式和无实际内容的时间线条目。
...
- 在微博轻享版“我的”页提供 ReWeibo 设置入口，可控制入口显示、新微博优先排序、首页缓存时间跨度，以及首页“跳转”和“删除”快捷按钮。
...
- Adds a “newest posts first” setting, enabled by default. Disabling it preserves Weibo Lite's native timeline order without affecting ad filtering, cache, jump/delete actions, reading position, gap filling, or preload.
...
- Adds a ReWeibo entry to Weibo Lite's profile drawer for controlling entry visibility, newest-first ordering, the home cache window, and the independent home “跳转” and “删除” shortcuts.
